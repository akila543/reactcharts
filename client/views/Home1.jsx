...
handlebikeId(e,{value}){
  ...
  let yeararr = [];
  console.log("total bike data",bikes);
  console.log("selected bike name",value);
  Request.get('http://localhost:8585/getbikedataforchart')
        ...
        .end((err, res) => {
          if (err) {
            ...
            console.log('res from bike data - > ',JSON.parse(res.text), typeof res.text);
            this.setState({
              selectedBikeDetails:JSON.parse(res.text)
            },()=>{
              this.state.selectedBikeDetails.map((item,key)=>{
                  ...
                  console.log("item ", item.data);
                    ...
                    item.data.map((item1,key1)=>{
                      ...
                      this.setState({yearArr:yeararr,finalselecteddata:item.data});
...
handleYear(e,{value}){
 console.log("selected year: ",value);
  console.log("final selected bike data: ",this.state.finalselecteddata);

  let selectedYear = value;
  let finalselecteddata = this.state.finalselecteddata;
  finalselecteddata.map((item,key)=>{
